            ...
            {activeTab === 'database' && (
                <div className="space-y-2 animate-in fade-in slide-in-from-top-2">
                    <div className="relative">
                        <Search size={16} className="absolute left-3 top-3 text-slate-400"/><input type="text" placeholder="Rechercher (Pseudo, Titre...)" className="w-full bg-slate-50 dark:bg-slate-800 pl-10 pr-4 py-2.5 rounded-xl text-sm border-none outline-none focus:ring-2 ring-blue-500/50 transition text-slate-900 dark:text-white" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)}/>
                    </div>
                    <label className="flex items-center gap-2 text-xs text-slate-500 dark:text-slate-400 cursor-pointer p-1"><input type="checkbox" checked={showOnlyReported} onChange={e => setShowOnlyReported(e.target.checked)} className="rounded border-slate-300 text-blue-600 focus:ring-blue-500"/><Filter size={12}/> Afficher uniquement les événements signalés</label>
                </div>
            )}
        </div>
                                ...
                                <div className="space-y-3">
                                    {details.eventReports.map((rep) => (
                                        <div key={rep.id} className="bg-red-50 dark:bg-red-900/10 p-4 rounded-xl flex justify-between items-start gap-4">
                                            <div><p className="text-sm font-bold text-red-800 dark:text-red-300">"{rep.reason}"</p><p className="text-xs text-red-600/70 mt-1">Par @{rep.profiles?.pseudo || 'Anonyme'} • {new Date(rep.created_at).toLocaleString()}</p></div>
                                            {rep.status === 'pending' ? (
                                                <div className="flex flex-col gap-2"><button onClick={() => closeReport(rep.id, 'resolved')} className="p-1.5 bg-green-100 text-green-700 rounded hover:bg-green-200"><CheckCircle size={16}/></button><button onClick={() => closeReport(rep.id, 'ignored')} className="p-1.5 bg-slate-200 text-slate-600 rounded hover:bg-slate-300"><XCircle size={16}/></button></div>
                                            ) : <span className={`text-[10px] font-bold px-2 py-1 rounded uppercase ${rep.status === 'resolved' ? 'bg-green-100 text-green-700' : 'bg-slate-100 text-slate-500'}`}>{rep.status === 'resolved' ? 'Traité' : 'Ignoré'}</span>}
                                        </div>
                                    ))}
                                </div>
                            </div>
                        ) : <div className="bg-green-50 dark:bg-green-900/20 p-4 rounded-xl border border-green-100 dark:border-green-900/30 text-green-700 dark:text-green-400 flex items-center gap-2 font-medium"><ShieldCheck size={20}/> Aucun signalement.</div>}
                             ...
                             {details.organizer ? (
                                <div className="space-y-6">
                                    <div className="flex items-center gap-4">
                                        <div className="w-16 h-16 bg-slate-100 dark:bg-slate-800 rounded-full flex items-center justify-center font-bold text-2xl text-slate-600 dark:text-slate-400">{details.organizer.pseudo?.[0]?.toUpperCase()}</div>
                                        <div><p className="font-bold text-xl text-slate-900 dark:text-white">@{details.organizer.pseudo}</p><span className={`px-3 py-1 rounded-full text-xs font-bold uppercase ${details.organizer.account_status === 'active' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>Statut : {details.organizer.account_status}</span></div>
                                    </div>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-100 dark:border-slate-800">
                                        <div><p className="text-xs text-slate-500 uppercase font-bold mb-1">Identité</p><p className="font-medium text-slate-900 dark:text-white">{details.organizer.prenom} {details.organizer.nom}</p></div>
                                        <div><p className="text-xs text-slate-500 uppercase font-bold mb-1">Date de naissance</p><p className="font-medium text-slate-900 dark:text-white">{new Date(details.organizer.date_naissance).toLocaleDateString()} <span className="text-slate-400 font-normal ml-2">({new Date().getFullYear() - new Date(details.organizer.date_naissance).getFullYear()} ans)</span></p></div>
                                        <div className="col-span-2"><p className="text-xs text-slate-500 uppercase font-bold mb-1">ID (Système)</p><p className="font-mono text-xs text-slate-400 truncate select-all">{details.organizer.id}</p></div>
                                    </div>
                                    <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-100 dark:border-slate-800 flex items-start gap-3">
                                        <div className="bg-white dark:bg-slate-900 p-2 rounded-lg text-slate-400 border border-slate-200 dark:border-slate-700"><ScanFace size={24} /></div>
                                        <div>
                                            <h4 className="font-bold text-sm text-slate-700 dark:text-slate-300 uppercase mb-1">Vérification CNI</h4>
                                            {details.organizer.cni_status === 'verified' ? <div className="flex items-center gap-2 text-green-600 font-bold text-sm"><CheckCircle size={16} /> Identité certifiée</div> : details.organizer.cni_status === 'rejected' ? <div className="flex items-center gap-2 text-red-500 font-bold text-sm"><XCircle size={16} /> Refusée (Suspect)</div> : <div className="flex items-center gap-2 text-slate-500 font-bold text-sm"><span className="w-2 h-2 rounded-full bg-slate-400"></span> Non effectué</div>}
                                        </div>
                                    </div>
                                    <div className="grid grid-cols-2 gap-2 pt-2"><button onClick={() => sanctionUser('suspended')} className="bg-orange-50 dark:bg-orange-900/20 text-orange-700 dark:text-orange-400 hover:bg-orange-100 dark:hover:bg-orange-900/30 p-2 rounded-lg text-sm font-bold transition">Suspendre (Avert.)</button><button onClick={() => sanctionUser('banned')} className="bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-400 hover:bg-red-100 dark:hover:bg-red-900/30 p-2 rounded-lg text-sm font-bold transition">BANNIR</button></div>
                                </div>
                        ...
                        <div className="bg-white dark:bg-slate-900 p-6 rounded-2xl shadow-sm border border-slate-200 dark:border-slate-800">
                            ...
                            {currentEvent?.is_cancelled && (
                                <div className="bg-red-50 dark:bg-red-900/10 border border-red-100 dark:border-red-900/30 p-4 rounded-xl mb-6">
                                    <div className="flex items-start gap-3">
                                        <AlertOctagon className="text-red-600 dark:text-red-400 shrink-0 mt-1" />
                                        <div>
                                            <h4 className="font-bold text-red-800 dark:text-red-300 uppercase mb-1">Événement Annulé</h4>
                                            <p className="text-xs text-red-600/80 dark:text-red-400/80 mb-2">Le {new Date(currentEvent.cancelled_at).toLocaleString()}</p>
                                            <div className="bg-white/50 dark:bg-black/20 p-2 rounded-lg">
                                                <p className="text-sm font-medium text-red-900 dark:text-red-200">Motif : "{currentEvent.cancellation_reason}"</p>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div className="max-h-40 overflow-y-auto border border-slate-100 dark:border-slate-700 rounded-xl">
                                {details.participants.map((p, i) => (
                                    <div key={i} className="flex justify-between p-3 hover:bg-slate-50 dark:hover:bg-slate-800 border-b border-slate-50 dark:border-slate-800 last:border-0"><span className="font-medium text-slate-800 dark:text-white">{p.profiles?.pseudo || p.guest_name}</span><span className="text-xs text-slate-400">{p.user_id ? 'Membre' : 'Invité'}</span></div>
                                ))}
